...
const PrivateChat = () => {
  ...
  const { chatId, otherUserEmail, accentColor } = route.params as { 
    chatId: string;
  ...
  //Set loaded messages
  useEffect(() => {
    const messagesRef = collection(FIREBASE_DB, 'privateChats', chatId, 'messages');
  ...
  const sendMessage = async () => {
    ...
    await addDoc(collection(FIREBASE_DB, 'privateChats', chatId, 'messages'), {
      text: input,
    ...
    await updateDoc(doc(FIREBASE_DB, 'privateChats', chatId), {
      lastMessage: input,
  ...
  return (
    <>
      <AppbarNested title={otherUserEmail} backgroundColor={accentColor} />
      <KeyboardAvoidingView
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={{ textAlign: 'center', color: 'darkgray', height: 50, fontSize: 20 }}>
              No messages yet!
            </Text>
          }
        />
        <View style={styles.inputContainer}>
          <TextInput
            ...
            style={styles.input}
          />
          <IconButton icon="send" iconColor="white" onPress={sendMessage} style={[styles.sendButton, { backgroundColor: accentColor }]} />
        </View>
      </KeyboardAvoidingView>
    </>
  );
